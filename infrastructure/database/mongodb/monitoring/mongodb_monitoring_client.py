...
class MongoDBMonitoringClient(IQueryMonitoringClient):
    def __init__(self, client: MongoClient, config: Optional[QueryMonitoringConfig] = None):
        ...
        self.obs = ObservabilityClient(service_name="mongodb-monitoring-client")
        
        self.collector = MongoDBQueryCollector(client, config)
        self.analyzer = MongoDBQueryAnalyzer(client)
        self.exporter = MongoDBMetricsExporter(client, config)
    ...
    def execute_with_monitoring(self, query: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            metric = QueryMetric(
                ...
                database="mongodb",
                collection_table=self._extract_collection_name(query),
                execution_time_ms=execution_time_ms,
                status=QueryStatus.SUCCESS,
                performance_level=performance_level,
                timestamp=datetime.utcnow(),
                affected_rows=self._extract_affected_rows(result),
                plan_details=self._get_query_plan(query, params) if self.config.enable_query_plans else None
            )
        ...
        try:
            ...
            for query in slow_queries:
                issues.append({
                    ...
                    "recommendation": f"Optimize query or add indexes for collection '{query.collection_table}'"
                })
    ...
    def _execute_query(self, query: str, params: Optional[Dict[str, Any]]) -> Any:
        if query.strip().startswith("find"):
            return self._execute_find_query(query, params)
        elif query.strip().startswith("aggregate"):
            return self._execute_aggregate_query(query, params)
        elif query.strip().startswith("insert"):
            return self._execute_insert_query(query, params)
        elif query.strip().startswith("update"):
            return self._execute_update_query(query, params)
        elif query.strip().startswith("delete"):
            return self._execute_delete_query(query, params)
        else:
            raise ValueError(f"Unsupported query type: {query[:50]}...")

    def _execute_find_query(self, query: str, params: Optional[Dict[str, Any]]) -> Any:
        collection_name, filter_dict = self._parse_find_query(query)
    ...
    def _execute_aggregate_query(self, query: str, params: Optional[Dict[str, Any]]) -> Any:
        collection_name, pipeline = self._parse_aggregate_query(query)
        if not collection_name:
            raise ValueError("Invalid aggregate query format")
        
        collection = self.client[collection_name]
        return list(collection.aggregate(pipeline or []))

    def _execute_insert_query(self, query: str, params: Optional[Dict[str, Any]]) -> Any:
        collection_name, documents = self._parse_insert_query(query)
        if not collection_name:
            raise ValueError("Invalid insert query format")
        
        collection = self.client[collection_name]
        return collection.insert_many(documents or [])

    def _execute_update_query(self, query: str, params: Optional[Dict[str, Any]]) -> Any:
        collection_name, filter_dict, update_dict = self._parse_update_query(query)
        if not collection_name:
            raise ValueError("Invalid update query format")
        
        collection = self.client[collection_name]
        return collection.update_many(filter_dict or {}, update_dict or {})

    def _execute_delete_query(self, query: str, params: Optional[Dict[str, Any]]) -> Any:
        collection_name, filter_dict = self._parse_delete_query(query)
        if not collection_name:
            raise ValueError("Invalid delete query format")
        
        collection = self.client[collection_name]
        return collection.delete_many(filter_dict or {})
    ...
    def _extract_affected_rows(self, result: Any) -> Optional[int]:
        if hasattr(result, 'acknowledged') and hasattr(result, 'inserted_ids'):
            return len(result.inserted_ids)
        elif hasattr(result, 'acknowledged') and hasattr(result, 'matched_count'):
            return result.matched_count
        elif isinstance(result, list):
            return len(result)
        return None

    def _get_query_plan(self, query: str, params: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        try:
            return self.analyzer.explain_query(query, "mongodb")
        except:
            return None
    ...
    def _generate_summary_recommendations(self, summary: Dict[str, Any], health: Dict[str, Any], index_gaps: List[Dict[str, Any]]) -> List[str]:
        ...
        if index_gaps:
            recommendations.append(f"Index gaps identified for {len(index_gaps)} collections")
    ...
    def _parse_find_query(self, query: str) -> tuple[Optional[str], Optional[Dict[str, Any]]]:
        from .mongodb_query_analyzer import MongoDBQueryAnalyzer
        analyzer = MongoDBQueryAnalyzer(self.client)
        return analyzer._parse_find_query(query)

    def _parse_aggregate_query(self, query: str) -> tuple[Optional[str], Optional[List[Dict[str, Any]]]]:
        from .mongodb_query_analyzer import MongoDBQueryAnalyzer
        analyzer = MongoDBQueryAnalyzer(self.client)
        return analyzer._parse_aggregate_query(query)

    def _parse_insert_query(self, query: str) -> tuple[Optional[str], Optional[List[Dict[str, Any]]]]:
        return None, None

    def _parse_update_query(self, query: str) -> tuple[Optional[str], Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
        return None, None, None

    def _parse_delete_query(self, query: str) -> tuple[Optional[str], Optional[Dict[str, Any]]]:
        return None, None
