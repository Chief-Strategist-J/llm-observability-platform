...
from ..client.cassandra_client import execute_query, CassandraConnectionConfig


class CassandraMonitoringClient(IQueryMonitoringClient):
    def __init__(self, session: Session, config: Optional[QueryMonitoringConfig] = None):
        ...
        self.obs = ObservabilityClient(service_name="cassandra-monitoring-client")
        
        self.collector = CassandraQueryCollector(session, config)
        self.analyzer = CassandraQueryAnalyzer(session)
        self.exporter = CassandraMetricsExporter(session, config)
    ...
    def execute_with_monitoring(self, query: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            result = execute_query(self.session, query, params)
    ...
    def get_database_health_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            schema_analysis = self.analyzer.get_schema_analysis("cassandra")
    ...
    def get_schema_performance_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(tables) > 100:
                recommendations.append("High number of tables detected - consider schema optimization")
            
            indexes = schema.get("indexes", [])
            if len(indexes) < len(tables) * 0.5:
                recommendations.append("Consider adding more indexes to improve query performance")
            
            keyspace = schema.get("keyspace", {})
            if keyspace.get("durable_writes", True):
                recommendations.append("Consider disabling durable writes for high-throughput scenarios")
            ...
            return {
                ...
                "analysis": {
                    ...
                    "keyspace_replication": keyspace.get("replication", {})
                },
                "timestamp": datetime.utcnow().isoformat()
            }
    ...
    def get_table_performance_analysis(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            table_info = table_stats.get("table_info", {})
            if table_info.get("bloom_filter_fp_chance", 1.0) > 0.1:
                recommendations.append(f"Consider optimizing bloom filter for table '{table_name}'")
            
            if table_info.get("compaction", {}).get("class", "") == "SizeTieredCompactionStrategy":
                recommendations.append(f"Consider using LeveledCompactionStrategy for table '{table_name}'")
            
            columns = table_stats.get("columns", [])
            if len(columns) > 50:
                recommendations.append(f"Table '{table_name}' has many columns - consider denormalization")
