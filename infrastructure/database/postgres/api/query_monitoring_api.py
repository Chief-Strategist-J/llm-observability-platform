...
from infrastructure.database.postgres.client.postgres_client import get_connection, PostgreSQLConnectionConfig
from .postgres_monitoring_client import PostgreSQLMonitoringClient
...
router = APIRouter(prefix="/postgres/queries", tags=["postgres-query-monitoring"])
obs = ObservabilityClient(service_name="postgres-query-monitoring-api")


class QueryExecutionRequest(BaseModel):
    query: str = Field(..., description="PostgreSQL SQL query to execute")
    params: Optional[Dict[str, Any]] = Field(None, description="Query parameters")


class QueryAnalysisRequest(BaseModel):
    query: str = Field(..., description="PostgreSQL SQL query to analyze")
...
def get_monitoring_client() -> PostgreSQLMonitoringClient:
    try:
        config = PostgreSQLConnectionConfig()
        connection = get_connection(config)
        monitoring_config = QueryMonitoringConfig()
        return PostgreSQLMonitoringClient(connection, monitoring_config)
...
async def analyze_query(request: QueryAnalysisRequest):
    ...
    try:
        config = PostgreSQLConnectionConfig()
        connection = get_connection(config)
        analyzer = PostgreSQLQueryAnalyzer(connection)
...
async def explain_query(
    query: str = Query(..., description="SQL query to explain"),
...
async def get_prometheus_metrics():
    ...
    try:
        config = PostgreSQLConnectionConfig()
        connection = get_connection(config)
        exporter = PostgreSQLMetricsExporter(connection)
...
async def get_schema_info(database: str = Query("scaibu_default", description="Database name")):
