...
async def configure_source_logs(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not config_path:
        logger.error("configure_source_logs: missing_config_path")
    ...
    try:
        ...
        if not src.exists() or not src.is_file():
            logger.error("configure_source_logs: source config not found: %s", str(src))
        ...
        try:
            if src.resolve() == target:
                logger.info("configure_source_logs: source and target are the same file (%s). No copy needed.", str(target))
            else:
                shutil.copy2(str(src), str(target))
                logger.info("configure_source_logs copied config from %s to %s", str(src), str(target))
        except Exception:
            shutil.copyfile(str(src), str(target))
            logger.info("configure_source_logs fallback copyfile used for %s -> %s", str(src), str(target))

        try:
            target.chmod(0o644)
        except Exception as e:
            logger.debug("configure_source_logs chmod warning: %s", e)

        if not target.exists():
            logger.error("configure_source_logs failed: target not present after copy: %s", str(target))
            return {"success": False, "data": None, "error": "copy_failed"}

        size = target.stat().st_size
        logger.info("configure_source_logs success target=%s size=%d", str(target), size)

        return {"success": True, "data": {"applied_config": str(target), "size": size}, "error": None}

    except Exception as e:
        logger.exception("configure_source_logs failed: %s", e)
