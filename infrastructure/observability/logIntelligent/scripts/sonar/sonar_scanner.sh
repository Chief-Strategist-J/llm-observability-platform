...
run_scan() {
  ...
  local token=$(curl -s -X POST -u admin:Scaibu@12345678 "http://localhost:9000/api/user_tokens/generate?name=log-intelligent-$(date +%s)" | grep -o '"token":"[^"]*"' | cut -d'"' -f4)
  ...
  if docker run --rm --network host \
    ...
    -Dsonar.language=py \
    -Dsonar.sources=api,client,domain,enrichment,ingestion,intelligence,storage \
    -Dsonar.tests=test \
