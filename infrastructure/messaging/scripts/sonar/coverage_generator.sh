...
generate_coverage() {
  ...
  # Use existing pytest configuration from pyproject.toml
  if python3 -m pytest --cov=infrastructure/messaging --cov-report=xml:$coverage_dir/coverage.xml --cov-report=html:$coverage_dir/html --cov-report=term tests/; then
