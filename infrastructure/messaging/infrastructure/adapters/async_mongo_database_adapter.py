...
class AsyncMongoDatabaseAdapter(DatabasePort):
    ...
    async def _ensure_indexes(self):
        ...
        await self._events_collection.create_index([("topic", ASCENDING), ("partition", ASCENDING), ("offset", ASCENDING)], unique=True)
        await self._offsets_collection.create_index([("consumer_group", ASCENDING)])
        await self._offsets_collection.create_index([("consumer_group", ASCENDING), ("topic", ASCENDING), ("partition", ASCENDING)], unique=True)
    ...
    async def save_event(self, event: EventRecord) -> str:
        ...
        existing = await self._events_collection.find_one({"topic": event.topic, "partition": event.partition, "offset": event.offset})
        return str(existing["_id"]) if existing else ""

    async def save_events_batch(self, events: List[EventRecord]) -> List[str]:
        ids = []
        for event in events:
            event_id = await self.save_event(event)
            ids.append(event_id)
        return ids
    ...
    async def get_events_by_topic(self, topic: str, limit: int = 100, offset: int = 0) -> List[EventRecord]:
        ...
        return [self._dict_to_event(doc) for doc in docs]
