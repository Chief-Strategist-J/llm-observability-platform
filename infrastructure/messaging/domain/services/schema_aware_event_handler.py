...
class SchemaAwareEventHandler:
    ...
    def process_kafka_record(self, record: ConsumerRecord, deserialize: bool = True) -> str:
        value = record.value
    ...
    def process_kafka_records_batch(self, records: List[ConsumerRecord], deserialize: bool = True) -> List[str]:
        event_ids = []
        for record in records:
            event_id = self.process_kafka_record(record, deserialize)
    ...
    def process_with_custom_logic(self, record: ConsumerRecord, processor: Callable[[Any], Any], deserialize: bool = True) -> Dict[str, Any]:
        event_id = self.process_kafka_record(record, deserialize)
        try:
