...
- **FastAPI API Ingestion**: `http://localhost:8000`
...
### 3. Remote Node Configuration (Ansible)
...
To trigger backups manually:
```bash
./packages/configs/observability-deploy/scripts/backup.sh
```
Daily automated backups are registered automatically in the user's host crontab when starting Docker Compose local stack.
