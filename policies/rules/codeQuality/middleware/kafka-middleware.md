...
```mermaid
flowchart TD
    subgraph ProducerPipeline ["Kafka Producer Pipeline"]
        ...
        P3 --> P4["4. withRetryProducer (Deadline Backoff and Jitter)"]
        P4 --> P5["5. withIdempotenceGuard (App-Level Dedupe Lock)"]
        P5 --> P6["6. withSchemaValidation (Schema Registry Check)"]
        P6 --> P7["7. withSerialization (Avro or Protobuf Codec)"]
        P7 --> P8["8. withPartitionKeySelection (Composite Key Strategy)"]
        P8 --> P9["9. Batch Accumulator and Compression"]
        P9 --> P10["10. Broker Connection Transport"]
    end

    subgraph ConsumerPipeline ["Kafka Consumer Pipeline"]
        C1["1. Broker Connection Pool"] --> C2["2. Consumer Group Coordinator (Rebalance and Heartbeat)"]
        C2 --> C3["3. Prefetch Fetcher Buffer (Per-Partition Bounded)"]
        C3 --> C4["4. Partitioned Worker Pool (1 Worker Per Partition)"]
        C4 --> C5["5. withDLQOnFailure (Outermost Catch and Retry Topics)"]
        ...
        C10 --> C11["11. withDeserialization (Codec Decode and Tombstone Check)"]
