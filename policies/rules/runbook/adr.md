...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ADR-002: Implement FastAPI service-to-service REST Query API for latency-engine
  Status:  Accepted
  Date:    2026-06-26
  ...
  ── FAILURE MODES WE ARE SOLVING ─────────────────────────────────────────────
  FM-CURRENT-1: No query surface exists for latency-engine. Telemetry aggregates in Redis/ClickHouse are inaccessible.
                Severity: CRITICAL — prevents usage of calculated stats.
  FM-CURRENT-2: Eager database connection setup in clickhouse client crashes FastAPI on startup when ClickHouse is offline during tests or server restarts.
                Severity: HIGH — makes service brittle and fails unit/integration test compilation.
                Resolution: Lazy client initialization inside ClickHouse adapter.
  ...
  Option A: FastAPI app running on a background thread within the consumer process
    Solves: FM-CURRENT-1, FM-CURRENT-2
    Pros: Reuses existing exposed HTTP port (8002), doesn't block Kafka consumer loop because it runs in a separate thread, leverages FastAPI for validation and JWT.
  Option B: Independent query service deployment
    Solves: FM-CURRENT-1
    Cons: Increases container deployment footprint, requires additional network management.

  ── DECISION ─────────────────────────────────────────────────────────────────
  Chosen: Option A (FastAPI app running via uvicorn in a daemon thread).
  Because: Reuses existing exposed port, keeps deployment footprint lightweight, and meets all performance, security, and contract constraints.

  ── FAILURE MODES CREATED ────────────────────────────────────────────────────
  FM-NEW-1: Port conflicts on port 8002
    Symptom:  Service fail-starts with port already in use.
    Detection: Startup logs contain port binding error.
    Prevention: HTTP port configurable via `HEALTH_PORT` environment variable.

  ── REVIEW TRIGGER ───────────────────────────────────────────────────────────
  Review if:
    HTTP query volume exceeds 10,000 requests/second (at that point, Option B becomes necessary).