...
1. **Centralized Messaging Tracer (`CentralMessagingTracer` in `@observability/shared-infra/tracing`)**:
...
2. **Centralized Base Traced Handler (`BaseTracedKafkaHandler`)**:
   - Abstract base handler in `@observability/shared-infra/tracing` that automatically wraps `handlePayload(payload, event, span)` in an OpenTelemetry child span (`Handler <eventName>`).
...
└── 1. UserAuthDomainService.signIn() [packages/node/auth/src/features/auth/services/user-auth.service.ts]
    └── 2. AuthEventProducer.publishUserSignedIn(payload) [packages/node/auth/src/shared/messaging/producers/auth-event.producer.ts]
        │
        ├── 3. CentralMessagingTracer.createProducerSpan("auth.events.v1", "USER_SIGNED_IN") [@observability/shared-infra/tracing/messaging-tracer.ts]
        ...
        └── 5. CentralizedKafkaClient.publishToTopic('auth.events.v1', payload, headers) [packages/node/core/src/kafka/kafka-client.ts]
            ...
            └── 6. AuthEventConsumer.subscribeToTopic('auth.events.v1') [packages/node/auth/src/shared/messaging/consumers/auth-event.consumer.ts]
                ...
                │   ├── CentralMessagingTracer.createConsumerSpan(event, "auth.events.v1") [@observability/shared-infra/tracing/messaging-tracer.ts]
                ...
                │
                └── 8. UserSignedInHandler extends BaseTracedKafkaHandler [@observability/shared-infra/tracing/traced-handler.ts]
    ...
    └── 9. CentralMessagingTracer.finishSpan(consumerSpan) [@observability/shared-infra/tracing/messaging-tracer.ts]
