...
def init_tracer() -> None:
    ...
    if os.getenv("SKIP_OTLP_EXPORTER") != "true":
        try:
            ...
            prov.add_span_processor(SimpleSpanProcessor(GRPCOTLPSpanExporter(endpoint=endpoint, insecure=True)))
        except ImportError:
            try:
                from opentelemetry.exporter.otlp.proto.http.trace_exporter import OTLPSpanExporter as HTTPOTLPSpanExporter
                endpoint = os.getenv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4318/v1/traces")
                prov.add_span_processor(SimpleSpanProcessor(HTTPOTLPSpanExporter(endpoint=endpoint)))
            except ImportError:
                pass
...
def trace_span(
    ...
    init_tracer()
    t = trace.get_tracer("slo-burn-worker")
    ...
    with t.start_as_current_span(name, context=parent_ctx) as span:
        ...
        span.set_attribute("service.name", "slo-burn-worker")
        ...
        span.set_attribute("feature.name", "slo_burn")
