...
from kafka_messaging_internal.shared.types.schema import SchemaType
...
class SchemaAwareEventService:
    ...
    async def process_kafka_record(self, record: Dict[str, Any], deserialize: bool = True) -> str:
        ...
        event = EventRecord(
            ...
            value=value,
            timestamp=datetime.fromtimestamp(record['timestamp'] / 1000, tz=timezone.utc) if record.get('timestamp') else datetime.now(timezone.utc),
