"""Kafka activity for Temporal workflows."""
...
KAFKA_YAML = Path(__file__).parent.parent.parent.parent.parent / "deploy" / "docker" / "docker-compose.dev.yaml"


@activity.defn(name="start_kafka_activity")
async def start_kafka_activity(params: dict) -> dict:
    """Start Kafka container activity."""
    with _tracer.start_as_current_span("start_kafka") as span:
        ...
        logger.info("event=kafka_start_begin instance_id=%d", params.get("instance_id", 0))
        ...
        try:
            with _tracer.start_as_current_span("start_kafka_container") as start_span:
                ...
                logger.info("event=kafka_start_success instance_id=%d", params.get("instance_id", 0))
            ...
            logger.error("event=kafka_start_failed instance_id=%d error=%s", params.get("instance_id", 0), str(e))
...
async def stop_kafka_activity(params: dict) -> dict:
    ...
    with _tracer.start_as_current_span("stop_kafka") as span:
        ...
        logger.info("event=kafka_stop_begin instance_id=%d", params.get("instance_id", 0))
        
        try:
            with _tracer.start_as_current_span("stop_kafka_container") as stop_span:
                ...
                logger.info("event=kafka_stop_success instance_id=%d", params.get("instance_id", 0))
            ...
            span.record_error(e)
            span.set_status(Status(StatusCode.ERROR, str(e)))
            logger.error("event=kafka_stop_failed instance_id=%d error=%s", params.get("instance_id", 0), str(e))
