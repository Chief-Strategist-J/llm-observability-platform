...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1 && [ -f "$COMPOSE_FILE" ]; then
    echo -e "\033[0;34m[toxicity] Cleaning up previous image...\033[0m"
    docker image rm docker-toxicity toxicity-toxicity --force >/dev/null 2>&1 || true
    echo -e "\033[0;34m[toxicity] Starting Docker container...\033[0m"
...
if [ -n "$VENV_DIR" ]; then
    ...
    echo -e "\033[1;33m[toxicity] Virtual environment missing. Creating .venv...\033[0m"
...
if [ -f "$PACKAGE_DIR/pyproject.toml" ] || [ -f "$PACKAGE_DIR/requirements.txt" ]; then
    if ! "$PYTHON_EXE" -c "import uvicorn, fastapi" 2>/dev/null; then
        echo -e "\033[1;33m[toxicity] Dependencies missing. Auto-installing packages...\033[0m"
...
export HEALTH_PORT="${HEALTH_PORT:-8008}"
...
if [ -f "$PACKAGE_DIR/scripts/migrate.sh" ]; then
    echo -e "\033[0;34m[toxicity] Executing database migrations...\033[0m"
    "$PACKAGE_DIR/scripts/migrate.sh" || true
fi

echo -e "\033[0;32m[toxicity] Starting service on port ${HEALTH_PORT}...\033[0m"
