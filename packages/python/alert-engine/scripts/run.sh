...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1 && [ -f "$COMPOSE_FILE" ]; then
    echo -e "\033[0;34m[alert-engine] Cleaning up previous image...\033[0m"
    docker image rm docker-alert-engine alert-engine-alert-engine --force >/dev/null 2>&1 || true
    echo -e "\033[0;34m[alert-engine] Starting Docker container...\033[0m"
...
if [ -n "$VENV_DIR" ]; then
    ...
    echo -e "\033[1;33m[alert-engine] Virtual environment missing. Creating .venv...\033[0m"
...
if [ -f "$PACKAGE_DIR/pyproject.toml" ] || [ -f "$PACKAGE_DIR/requirements.txt" ]; then
    if ! "$PYTHON_EXE" -c "import fastapi" 2>/dev/null; then
        echo -e "\033[1;33m[alert-engine] Dependencies missing. Auto-installing packages...\033[0m"
...
export HEALTH_PORT="${HEALTH_PORT:-8004}"
...
if [ -f "$PACKAGE_DIR/scripts/migrate.sh" ]; then
    echo -e "\033[0;34m[alert-engine] Executing database migrations...\033[0m"
    "$PACKAGE_DIR/scripts/migrate.sh" || true
fi

echo -e "\033[0;32m[alert-engine] Starting service on port ${HEALTH_PORT}...\033[0m"
