...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1 && [ -f "$COMPOSE_FILE" ]; then
    echo -e "\033[0;34m[event-cost] Cleaning up previous image...\033[0m"
    docker image rm docker-event-cost event-cost-event-cost --force >/dev/null 2>&1 || true
    echo -e "\033[0;34m[event-cost] Starting Docker container...\033[0m"
...
if [ -n "$VENV_DIR" ]; then
    ...
    echo -e "\033[1;33m[event-cost] Virtual environment missing. Creating .venv...\033[0m"
...
if [ -f "$PACKAGE_DIR/pyproject.toml" ] || [ -f "$PACKAGE_DIR/requirements.txt" ]; then
    if ! "$PYTHON_EXE" -c "import fastapi" 2>/dev/null; then
        echo -e "\033[1;33m[event-cost] Dependencies missing. Auto-installing packages...\033[0m"
...
export HEALTH_PORT="${HEALTH_PORT:-8015}"
...
if [ -f "$PACKAGE_DIR/scripts/migrate.sh" ]; then
    echo -e "\033[0;34m[event-cost] Executing database migrations...\033[0m"
    "$PACKAGE_DIR/scripts/migrate.sh" || true
fi

echo -e "\033[0;32m[event-cost] Starting service on port ${HEALTH_PORT}...\033[0m"
