...
def test_all_scores_present_perfect():
    ...
    assert weights["coherence"] == pytest.approx(0.30)
    assert weights["faithfulness"] == pytest.approx(0.40)
    assert weights["toxicity"] == pytest.approx(0.20)
    assert weights["perplexity"] == pytest.approx(0.10)
