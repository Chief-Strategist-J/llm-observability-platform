...
if [ -n "$VENV_DIR" ]; then
    ...
    echo -e "\033[1;33m[forecast-worker] Virtual environment missing. Creating .venv...\033[0m"
...
if [ -f "$PACKAGE_DIR/pyproject.toml" ] || [ -f "$PACKAGE_DIR/requirements.txt" ]; then
    if ! "$PYTHON_EXE" -c "import fastapi" 2>/dev/null; then
        echo -e "\033[1;33m[forecast-worker] Dependencies missing. Auto-installing packages...\033[0m"
