...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1 && [ -f "$COMPOSE_FILE" ]; then
    echo -e "\033[0;34m[queue-embedding-worker] Cleaning up previous image...\033[0m"
    docker image rm docker-queue-embedding-worker queue-embedding-worker-queue-embedding-worker --force >/dev/null 2>&1 || true
    echo -e "\033[0;34m[queue-embedding-worker] Starting Docker container...\033[0m"
...
if [ -n "$VENV_DIR" ]; then
    ...
    echo -e "\033[1;33m[queue-embedding-worker] Virtual environment missing. Creating .venv...\033[0m"
...
if [ -f "$PACKAGE_DIR/pyproject.toml" ] || [ -f "$PACKAGE_DIR/requirements.txt" ]; then
    if ! "$PYTHON_EXE" -c "import uvicorn, fastapi" 2>/dev/null; then
        echo -e "\033[1;33m[queue-embedding-worker] Dependencies missing. Auto-installing packages...\033[0m"
...
export HEALTH_PORT="${HEALTH_PORT:-8010}"
...
if [ -f "$PACKAGE_DIR/scripts/migrate.sh" ]; then
    echo -e "\033[0;34m[queue-embedding-worker] Executing database migrations...\033[0m"
    "$PACKAGE_DIR/scripts/migrate.sh" || true
fi

echo -e "\033[0;32m[queue-embedding-worker] Starting service on port ${HEALTH_PORT}...\033[0m"
if [ -f "$PACKAGE_DIR/src/worker/index.py" ]; then
