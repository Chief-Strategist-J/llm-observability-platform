...
\subsection{Lack of Statistical Quality Evaluation}
...
\begin{itemize}[noitemsep]
\item Precision and Recall across various sequence lengths.
\item F1-score and ROC-AUC under prefix/suffix/middle toxicity distributions.
\item False Positive Rate (FPR) and False Negative Rate (FNR) to quantify the impact of middle-truncation.
\end{itemize}
...
The toxicity probability scores for a standard clean test sentence are visualized in Figure \ref{fig:scores}. The latency profile demonstrating the step transition at 510 tokens and subsequent linear growth with a small positive gradient is depicted in Figure \ref{fig:latency}.
