...
\definecolor{datacolor}{rgb}{0.8,0.7,0.0}
...
\lstdefinestyle{mystyle}{
  ...
  numbers=left,numbersep=5pt,showspaces=false,tabsize=4}
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Compute scaled query-key dot product attention
...
class PretrainingCausalLM(nn.Module):
    ...
    def forward(self, x, targets=None):
        # x: hidden states or token embeddings, shape: (batch_size, seq_len, d_model)
        ...
        if targets is not None:
            ...
            loss = self.loss_fn(shift_logits.view(-1, shift_logits.size(-1)), shift_targets.view(-1))
...
class SFTDatasetCollator:
    def __init__(self, tokenizer, ignore_index=-100):
        self.tokenizer = tokenizer
        self.ignore_index = ignore_index

    def __call__(self, batch):
        # batch: list of dicts with 'prompt' and 'response' strings
        input_ids_list = []
        labels_list = []
        
        for item in batch:
            prompt_tokens = self.tokenizer.encode(item['prompt'])
            res_tokens = self.tokenizer.encode(item['response'])
            
            # Concatenate prompt + response
            input_ids = prompt_tokens + res_tokens
            # Mask the prompt tokens in the label
            labels = [self.ignore_index] * len(prompt_tokens) + res_tokens
            
            input_ids_list.append(torch.tensor(input_ids))
            labels_list.append(torch.tensor(labels))
            
        # Pad sequences to max length in batch
        padded_inputs = nn.utils.rnn.pad_sequence(input_ids_list, batch_first=True, padding_value=self.tokenizer.pad_token_id)
        padded_labels = nn.utils.rnn.pad_sequence(labels_list, batch_first=True, padding_value=self.ignore_index)
        
        return padded_inputs, padded_labels

class SFTModuleLoss(nn.Module):
    def __init__(self, ignore_index=-100):
        super().__init__()
        self.ce = nn.CrossEntropyLoss(ignore_index=ignore_index)

    def forward(self, logits, targets):
        # Shifting logits and targets for next-token prediction
        shift_logits = logits[..., :-1, :].contiguous()
        shift_targets = targets[..., 1:].contiguous()
        return self.ce(shift_logits.view(-1, shift_logits.size(-1)), shift_targets.view(-1))
\end{lstlisting}
\newpage
%% ------------------------------------------------------------
...
class InstructionFormatter(nn.Module):
    def __init__(self, tokenizer, ignore_index=-100):
        super().__init__()
        self.tokenizer = tokenizer
        self.ignore_index = ignore_index
    ...
    def format_chatml(self, instruction, context, response):
        ...
        targets = [self.ignore_index] * len(prompt_ids) + response_ids
...
class GradNormMultiTaskTrainer:
    def __init__(self, model, num_tasks, alpha=0.12, lr_w=0.025):
        self.model = model
        ...
        self.w = nn.Parameter(torch.ones(num_tasks, requires_grad=True))
        self.initial_losses = None
        self.optimizer_w = torch.optim.Adam([self.w], lr=lr_w)

    def train_step(self, task_losses, shared_weights_layer, optimizer_model):
        # task_losses: tensor of shape (num_tasks,) containing current task losses
        if self.initial_losses is None:
            self.initial_losses = task_losses.detach().clone()
            
        weighted_losses = self.w * task_losses
        total_loss = weighted_losses.sum()
        
        # Backward pass on the main model parameters
        optimizer_model.zero_grad()
        total_loss.backward(retain_graph=True)
        
        # Compute gradient norms G_k for each task with respect to shared weights
        norms = []
        for i in range(self.num_tasks):
            grads = torch.autograd.grad(weighted_losses[i], list(shared_weights_layer.parameters()), retain_graph=True, create_graph=True)
            # Accumulate squared norms of all parameters in shared weights layer
            norm = torch.sqrt(sum(g.pow(2).sum() for g in grads if g is not None))
            norms.append(norm)
        norms = torch.stack(norms)
        
        # Calculate loss ratios and target gradient norms
        loss_ratios = task_losses.detach() / self.initial_losses
        mean_norm = norms.mean().detach()
        mean_ratio = loss_ratios.mean()
        
        targets = mean_norm * (loss_ratios / mean_ratio) ** self.alpha
        
        # Update task weights w
        self.optimizer_w.zero_grad()
        grad_loss = torch.sum(torch.abs(norms - targets))
        grad_loss.backward()
        
        # Update weights and normalize them
        self.optimizer_w.step()
        with torch.no_grad():
            self.w.copy_(self.w / self.w.sum() * self.num_tasks)
            
        optimizer_model.step()
        return total_loss.item(), self.w.detach().tolist()
\end{lstlisting}
\newpage
%% ------------------------------------------------------------
...
class CoTGenerator(nn.Module):
    ...
    def sample_top_p(self, logits, top_p=0.9, temperature=0.7):
        ...
        # Remove tokens with cumulative probability above top_p (but keep the first one)
        ...
        # Re-compute probabilities and sample
        filtered_probs = F.softmax(scaled_logits, dim=-1)
        next_token = torch.multinomial(filtered_probs, num_samples=1)
        return next_token

    def generate(self, input_ids, max_len=100, top_p=0.9, temp=0.7):
        # Autoregressive generation loop
        generated = input_ids.clone()
        for _ in range(max_len):
            outputs, _ = self.model(generated)
            next_logit = outputs[:, -1, :]
            next_token = self.sample_top_p(next_logit, top_p=top_p, temperature=temp)
...
class StepByStepLossAndConfidence(nn.Module):
    def __init__(self, step_token_id=50257, ignore_index=-100):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        batch_size, seq_len = shift_targets.size()
        
        flat_logits = shift_logits.view(-1, shift_logits.size(-1))
        flat_targets = shift_targets.view(-1)
        
        # Calculate loss per token position
        losses = self.ce(flat_logits, flat_targets).view(batch_size, seq_len)
        
        step_perplexities = []
        for b in range(batch_size):
            step_indices = (shift_targets[b] == self.step_token_id).nonzero(as_tuple=True)[0]
            start = 0
            for idx in step_indices:
                step_loss = losses[b, start:idx.item()]
                # Exclude masked tokens
                valid_losses = step_loss[shift_targets[b, start:idx.item()] != self.ignore_index]
                if len(valid_losses) > 0:
                    step_ppl = torch.exp(valid_losses.mean())
                    step_perplexities.append(step_ppl.item())
                start = idx.item() + 1
                
        mean_loss = losses[shift_targets != self.ignore_index].mean()
        return mean_loss, step_perplexities
\end{lstlisting}
\newpage
%% ------------------------------------------------------------
...
class ProcessRewardModel(nn.Module):
    def __init__(self, hidden_dim, step_token_id=50257):
        super().__init__()
        self.step_token_id = step_token_id
    ...
    def forward(self, hidden_states, input_ids, step_labels=None):
        # hidden_states shape: (batch_size, seq_len, hidden_dim)
        # input_ids shape: (batch_size, seq_len)
        # step_labels: tensor containing float correctness labels {0.0, 1.0} at step boundaries, shape: (batch_size, seq_len)
        ...
        if step_labels is not None:
            # Mask out non-step boundary token positions in loss calculation
            valid_logits = logits[step_mask]
            valid_labels = step_labels[step_mask]
            loss = self.bce(valid_logits, valid_labels)
            
        # Compute predicted step probabilities
        probs = torch.sigmoid(logits) * step_mask.float()
        return probs, loss
\end{lstlisting}
\newpage
%% ============================================================
...
class LoRALinear(nn.Module):
    def __init__(self, in_features, out_features, r=8, alpha=16):
        ...
        nn.init.zeros_(self.lora_B) # B is initialized to 0, making Delta W = 0 initially

    def merge_weights(self):
        # Update base weights directly
        with torch.no_grad():
            self.linear.weight.add_(self.scaling * (self.lora_B @ self.lora_A))

    def unmerge_weights(self):
        # Subtract back the low rank product
        with torch.no_grad():
            self.linear.weight.sub_(self.scaling * (self.lora_B @ self.lora_A))

    def forward(self, x):
        # Compute forward pass: W0*x + (alpha/r)*B*A*x
...
class QLoRALinear(nn.Module):
    def __init__(self, in_features, out_features, r=8, alpha=16, block_size=64):
        ...
        # Simulated NF4 weights stored as half precision, frozen
        self.weight_nf4 = nn.Parameter(torch.randn(out_features, in_features).half(), requires_grad=False)
        # Block-wise scales for dequantization
        self.scales = nn.Parameter(torch.zeros(out_features, (in_features + block_size - 1) // block_size).half(), requires_grad=False)
        self.compute_simulated_scales()
    ...
    def compute_simulated_scales(self):
        # Group weights into blocks and compute max values
        w = self.weight_nf4.data
        for i in range(self.scales.size(1)):
            start = i * self.block_size
            end = min(start + self.block_size, self.in_features)
            block = w[:, start:end]
            self.scales[:, i] = torch.max(torch.abs(block), dim=-1).values

    def dequantize(self):
        # Dequantize simulated NF4 block-by-block
        dequantized = torch.zeros_like(self.weight_nf4)
        for i in range(self.scales.size(1)):
            start = i * self.block_size
            end = min(start + self.block_size, self.in_features)
            scale = self.scales[:, i].unsqueeze(-1)
            dequantized[:, start:end] = self.weight_nf4[:, start:end] * scale
        return dequantized

    def forward(self, x):
        # Dequantize base weights on the fly
        weight = self.dequantize()
        base_out = torch.nn.functional.linear(x, weight)
        lora_out = (x @ self.lora_A.t() @ self.lora_B.t()) * self.scaling
        return base_out + lora_out
\end{lstlisting}
\newpage
...
class AdaLoRALinear(nn.Module):
    def __init__(self, in_features, out_features, r=8, gamma=0.1):
        super().__init__()
        self.linear = nn.Linear(in_features, out_features)
        ...
        self.lora_P = nn.Parameter(torch.randn(out_features, r))
        self.lora_E = nn.Parameter(torch.randn(r)) # Singular values
        self.lora_Q = nn.Parameter(torch.randn(r, in_features))
    ...
    def compute_orthogonality_loss(self):
        # P^T * P - I
        p_orth = torch.matmul(self.lora_P.t(), self.lora_P) - torch.eye(self.r, device=self.lora_P.device)
        # Q * Q^T - I
        q_orth = torch.matmul(self.lora_Q, self.lora_Q.t()) - torch.eye(self.r, device=self.lora_Q.device)
        return self.gamma * (torch.norm(p_orth, p='fro')**2 + torch.norm(q_orth, p='fro')**2)

    def prune_rank(self, keep_mask):
        # keep_mask: boolean tensor of shape (r,) indicating active singular values
        with torch.no_grad():
            self.lora_E.copy_(self.lora_E * keep_mask.float())

    def forward(self, x):
        base_out = self.linear(x)
        # Delta W = P * diag(E) * Q
        delta_W = self.lora_P @ torch.diag(self.lora_E) @ self.lora_Q
...
class DoRALinear(nn.Module):
    def __init__(self, in_features, out_features, r=8, alpha=16):
        ...
        # Directional adapter matrices
        self.lora_A = nn.Parameter(torch.zeros(r, in_features))
        self.lora_B = nn.Parameter(torch.zeros(out_features, r))
        nn.init.kaiming_uniform_(self.lora_A, a=5**0.5)
        nn.init.zeros_(self.lora_B)
        
        # Magnitude parameters m, initialized to column norm of base weights
        W0 = self.linear.weight.data
        self.m = nn.Parameter(torch.norm(W0, p=2, dim=1))

    def forward(self, x):
        W0 = self.linear.weight
        delta_W = (self.lora_B @ self.lora_A) * self.scaling
        
        # Calculate updated direction matrix V
        V = W0 + delta_W
        V_norm = torch.norm(V, p=2, dim=1, keepdim=True)
        
        # Construct weight matrix with separate magnitude and direction components
        W = self.m.unsqueeze(1) * (V / V_norm)
...
class IA3AttentionWrapper(nn.Module):
    def __init__(self, key_dim, value_dim):
        super().__init__()
        # Scaling vectors initialized to ones
        self.l_K = nn.Parameter(torch.ones(key_dim))
        self.l_V = nn.Parameter(torch.ones(value_dim))

    def scale_keys(self, K):
        # K shape: (batch_size, num_heads, seq_len, head_dim) or (batch_size, seq_len, key_dim)
        return K * self.l_K

    def scale_values(self, V):
        return V * self.l_V

class IA3FFNWrapper(nn.Module):
    def __init__(self, ffn_dim):
        super().__init__()
        self.l_FFN = nn.Parameter(torch.ones(ffn_dim))

    def scale_hidden(self, hidden_states):
        # hidden_states: intermediate feedforward layer activations
        return hidden_states * self.l_FFN
\end{lstlisting}
\newpage
%% ------------------------------------------------------------
...
class BottleneckAdapter(nn.Module):
    def __init__(self, d_model, bottleneck_dim=16):
        ...
        # Initialize up projection to zero to act as identity mapping at start
...
class PrefixTuningAttention(nn.Module):
    def __init__(self, d_model, num_heads, prefix_len=10):
        super().__init__()
        self.d_model = d_model
        self.num_heads = num_heads
        self.head_dim = d_model // num_heads
    ...
    def forward(self, x, past_keys=None, past_values=None):
        # past_keys, past_values: prefix cache parameters, shape (batch_size, num_heads, prefix_len, head_dim)
        b, t, c = x.size()
        ...
        if past_keys is not None and past_values is not None:
            # Prepend virtual prefix keys/values to the computed sequences
...
class PromptTuningEmbedding(nn.Module):
    def __init__(self, base_embeddings, prompt_len=10):
        super().__init__()
        self.base_embeddings = base_embeddings
        for p in self.base_embeddings.parameters():
            p.requires_grad = False
            
        self.prompt_len = prompt_len
        self.prompt_embeddings = nn.Parameter(torch.randn(prompt_len, base_embeddings.embedding_dim))

    def forward(self, input_ids):
        # input_ids shape: (batch_size, seq_len)
        batch_size = input_ids.size(0)
        base_embs = self.base_embeddings(input_ids) # (batch_size, seq_len, d_emb)
        
        # Expand prompt embeddings to batch size
        prompts = self.prompt_embeddings.unsqueeze(0).expand(batch_size, -1, -1) # (batch_size, prompt_len, d_emb)
        
        # Concatenate prompt prefix with token embeddings
        return torch.cat([prompts, base_embs], dim=1)
\end{lstlisting}
\newpage
%% ------------------------------------------------------------
\section{P-Tuning v2}
...
class PTuningV2Model(nn.Module):
    def __init__(self, num_layers, num_heads, head_dim, prefix_len=10):
        ...
        # Directly optimized layer-wise prefixes
        self.prefixes = nn.ParameterList([
            nn.Parameter(torch.randn(2, num_heads, prefix_len, head_dim))
            for _ in range(num_layers)
        ])

    def forward(self, batch_size, layer_idx):
        layer_prefixes = self.prefixes[layer_idx]
        return layer_prefixes.unsqueeze(1).expand(-1, batch_size, -1, -1, -1)
\end{lstlisting}
\end{document}
