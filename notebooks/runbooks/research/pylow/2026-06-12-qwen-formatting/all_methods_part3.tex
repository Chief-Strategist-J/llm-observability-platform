...
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[datblk] (seed) {\textbf{Seed Pool} \\ ($\mathcal{S}$ (175 seeds))};
  \node[grayblk, right=of seed] (sample) {\textbf{Sample $k$} \\ (Instructions)};
  \node[trnblk, right=of sample] (gen) {\textbf{LLM} \\ \textbf{Generates} \\ ($(\hat{x},\hat{i},\hat{y})$)};
  \node[orangeblk, right=of gen] (filt) {\textbf{Quality} \\ \textbf{Filter} \\ (ROUGE $< \delta$)};
  \node[datblk, right=of filt] (pool) {\textbf{Expanded} \\ (Pool $\mathcal{S}'$)};
  \node[mrgblk, below=0.9cm of pool] (sft) {\textbf{SFT on} \\ ($\mathcal{S}'$)};
  \node[trnblk, left=2.6cm of sft] (better) {\textbf{Better} \\ (LLM $\theta'$)};
...
\begin{tikzpicture}[node distance=0.6cm and 1.0cm]
  \node[datblk] (orig) {\textbf{Original} \\ (Instruction $x$)};
  \node[trnblk, above right=0.5cm and 1.2cm of orig] (depth) {\textbf{In-Depth} \\ (Mutation)};
  \node[trnblk, below right=0.5cm and 1.2cm of orig] (breadth) {\textbf{In-Breadth} \\ (Mutation)};
  \node[grayblk, right=3.0cm of orig] (evolved) {\textbf{Evolved} \\ ($\hat{x}$)};
  \node[orangeblk, right=of evolved] (val) {\textbf{Validate} \\ (reject refusals)};
  \node[datblk, right=of val] (pool) {\textbf{Evolved} \\ (Dataset)};
  \node[mrgblk, below=0.8cm of pool] (sft) {\textbf{SFT} \\ (WizardLM style)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.6cm and 1.0cm]
  \node[datblk] (seed) {\textbf{Seed Code} \\ (Problems)};
  \node[trnblk, right=of seed] (mutate) {\textbf{Code} \\ (Mutator)};
  \node[grayblk, right=of mutate] (prob) {\textbf{Evolved} \\ (Problem $\hat{p}$)};
  \node[orangeblk, right=of prob] (solve) {\textbf{Teacher} \\ (LLM Solves)};
  \node[grayblk, right=of solve] (sol) {\textbf{Solution $\hat{s}$}};
  \node[cyanblk, below=0.9cm of sol] (test) {\textbf{Unit Test} \\ (Execution)};
  \node[datblk, left=1.4cm of test] (pass) {\textbf{Pass: add to} \\ ($\mathcal{D}_{\text{code}}$)};
  \node[lossblk, right=0.8cm of test] (fail) {\textbf{Fail: discard}};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[datblk] (schema) {\textbf{Schema /} \\ (Topic Seeds)};
  \node[frzblk, right=of schema] (teacher) {\textbf{Teacher Model} \\ ($P_T$ (large))};
  \node[grayblk, right=of teacher] (synth) {\textbf{Synthetic} \\ (Pairs $(x,y)$)};
  \node[orangeblk, right=of synth] (score) {\textbf{Quality} \\ (Scorer)};
  \node[datblk, right=of score] (final) {\textbf{Filtered} \\ ($\mathcal{D}_{\text{final}}$)};
  \node[trnblk, below=0.9cm of final] (student) {\textbf{Train Student} \\ ($P_S$ (small))};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[datblk] (x) {\textbf{Prompts $\{x_i\}$}};
  \node[trnblk, right=of x] (pol) {\textbf{Policy $\pi_\theta$} \\ (Sample $N$)};
  \node[grayblk, right=of pol, minimum height=2.0cm] (cands) {\textbf{$N$ Candidates} \\ (per prompt)};
  \node[orangeblk, right=of cands] (rm) {\textbf{Reward Model} \\ ($r(x,y)$)};
  \node[datblk, right=of rm] (keep) {\textbf{Keep} \\ ($r \geq \tau$)};
  \node[lossblk, below=0.9cm of cands] (drop) {\textbf{Discard} \\ ($r < \tau$)};
  \node[mrgblk, right=0.8cm of keep] (sft) {\textbf{SFT on} \\ (accepted)};
...
\begin{tikzpicture}[node distance=0.5cm and 0.9cm]
  \node[datblk,minimum width=1.8cm] (x) {\textbf{Prompt $x$}};
  \node[trnblk,minimum width=1.6cm, above right=0.3cm and 1.0cm of x] (y1) {\textbf{$y^{(1)}$}};
  \node[trnblk,minimum width=1.6cm, right=1.0cm of x] (y2) {\textbf{$y^{(2)}$}};
  \node[trnblk,minimum width=1.6cm, below right=0.3cm and 1.0cm of x] (yN) {\textbf{$y^{(N)}$}};
  \node[orangeblk, right=1.6cm of y2] (rm) {\textbf{Reward} \\ ($r(x,y^{(k)})$)};
  \node[mrgblk, right=of rm] (best) {\textbf{$y^*=\arg\max$}};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.6cm and 1.0cm]
  \node[datblk] (raw) {\textbf{Raw Corpus} \\ (noisy, large)};
  \node[grayblk, right=of raw] (ppl) {\textbf{Perplexity} \\ (Filter)};
  \node[grayblk, right=of ppl] (tox) {\textbf{Toxicity} \\ (Classifier)};
  \node[grayblk, right=of tox] (lang) {\textbf{Language} \\ (ID)};
  \node[orangeblk, right=of lang] (thresh) {\textbf{Threshold} \\ (Gates $\tau_m$)};
  \node[trnblk, above right=0.4cm and 1.0cm of thresh] (pass) {\textbf{PASS} \\ ($\rightarrow$ keep)};
  \node[lossblk, below right=0.4cm and 1.0cm of thresh] (fail) {\textbf{FAIL} \\ ($\rightarrow$ drop)};
  \node[datblk, right=1.8cm of thresh] (clean) {\textbf{Filtered} \\ ($\mathcal{D}_{\text{clean}}$)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[datblk] (corpus) {\textbf{Corpus} \\ ($\mathcal{D}$)};
  \node[grayblk, right=of corpus] (ngrams) {\textbf{N-gram} \\ (Shingles)};
  \node[trnblk, right=of ngrams] (minhash) {\textbf{MinHash} \\ (Signatures)};
  \node[grayblk, right=of minhash] (lsh) {\textbf{LSH} \\ (Bucketing)};
  \node[orangeblk, right=of lsh] (cmp) {\textbf{Pairwise} \\ ($J > \tau_J$?)};
  \node[datblk, above right=0.4cm and 1.0cm of cmp] (keep) {\textbf{Keep (unique)}};
  \node[lossblk, below right=0.4cm and 1.0cm of cmp] (drop) {\textbf{Drop (dup.)}};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[datblk] (unlabeled) {\textbf{Unlabelled} \\ (Pool $\mathcal{U}$)};
  \node[frzblk, right=of unlabeled] (model) {\textbf{Current} \\ (Model $\theta$)};
  \node[orangeblk, right=of model] (unc) {\textbf{Uncertainty} \\ (Estimator)};
  \node[grayblk, right=of unc] (select) {\textbf{Select top-$b$} \\ (examples)};
  \node[datblk, right=of select] (annot) {\textbf{Human} \\ (Annotator)};
  \node[trnblk, below=0.9cm of annot] (retrain) {\textbf{Retrain} \\ (on $\mathcal{L}$)};
  \node[mrgblk, left=2.4cm of retrain] (better) {\textbf{Better} \\ ($\theta'$)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.6cm and 1.0cm]
  \node[datblk] (all) {\textbf{Full Dataset} \\ ($\mathcal{D}$)};
  \node[grayblk, right=of all] (score) {\textbf{Difficulty} \\ (Scorer $d(\cdot)$)};
  \node[orangeblk, right=of score] (sort) {\textbf{Sort by} \\ (Difficulty)};
  \node[trnblk, right=of sort] (easy) {\textbf{Easy} \\ (Batch $t{=}0$)};
  \node[frzblk, below=0.5cm of easy] (med) {\textbf{Medium} \\ (Batch $t{=}T/2$)};
  \node[mrgblk, below=0.5cm of med] (hard) {\textbf{Hard} \\ (Batch $t{=}T$)};
  \node[lossblk, right=1.6cm of med] (model) {\textbf{Model} \\ (Training)};
...
\subsection{Mathematical Foundation}
Knowledge distillation transfers dark knowledge from a large teacher model $f_{\text{teacher}}$ to a compact student model $f_{\text{student}}$. Let $z_s$ and $z_t$ be the logits of the student and teacher models respectively. The soft probability distribution at temperature $T$ is computed as:
\begin{equation}
...
\begin{equation}
  \mathcal{L}_{\text{KD}} = (1-\alpha) \mathcal{L}_{\text{CE}}(y, p(z_s, 1)) + \alpha T^2 D_{\text{KL}}\bigl( p(z_t, T) \parallel p(z_s, T) \bigr)
\end{equation}
...
\begin{equation}
  \frac{\partial \mathcal{L}_{\text{KL}}}{\partial z_{s,i}} = \frac{1}{T} \bigl( p_i(z_s, T) - p_i(z_t, T) \bigr)
\end{equation}
...
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  ...
  \node[frzblk, above right=0.5cm and 1.2cm of x] (teacher) {\textbf{Teacher $P_T$} \\ (frozen, large)};
  \node[trnblk, below right=0.5cm and 1.2cm of x] (student) {\textbf{Student $P_S$} \\ (trainable, small)};
  \node[lossblk, right=3.0cm of x] (kl) {\textbf{KL Div} \\ ($\tau^2\,\text{KL}[P_T^\tau \| P_S^\tau]$)};
  \node[lossblk, below=0.7cm of kl] (ce) {\textbf{CE Loss} \\ ($\mathcal{L}_{\text{CE}}(P_S, y)$)};
  \node[mrgblk, right=2.0cm of $(kl)!0.5!(ce)$] (total) {\textbf{$\mathcal{L}_{\text{KD}}$} \\ (combined)};
...
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (prompts) {\textbf{Prompts $\{x_i\}$}};
  \node[frzblk, right=of prompts] (teacher) {\textbf{Teacher $P_T$} \\ (API or local)};
  \node[datblk, right=of teacher] (responses) {\textbf{Teacher} \\ (Responses $y_T$)};
  \node[trnblk, right=of responses] (student) {\textbf{Student $P_S$} \\ (SFT on $y_T$)};
  \node[lossblk, below=0.9cm of student] (loss) {\textbf{CE Loss} \\ ($-\log P_S(y_T|x)$)};
  \node[mrgblk, left=1.4cm of loss] (out) {\textbf{Distilled} \\ (Student)};
...
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  ...
  \node[frzblk, right=of x] (teacher) {\textbf{Teacher $P_T$}};
  \node[grayblk, right=of teacher] (cot) {\textbf{Chain $c_T$} \\ (+ Answer $y_T$)};
  \node[trnblk, below=1.0cm of teacher] (student) {\textbf{Student $P_S$}};
  \node[lossblk, right=of student] (loss) {\textbf{CoT + Answer} \\ (CE Loss)};
  \node[mrgblk, right=of loss] (out) {\textbf{Reasoning} \\ (Student)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[datblk] (x) {\textbf{Context $(x, y_{<t})$}};
  \node[frzblk, above right=0.5cm and 1.3cm of x] (tpol) {\textbf{Teacher} \\ \textbf{$\pi_T(\cdot|x,y_{<t})$} \\ (full distribution)};
  \node[trnblk, below right=0.5cm and 1.3cm of x] (spol) {\textbf{Student} \\ \textbf{$\pi_S(\cdot|x,y_{<t})$} \\ (full distribution)};
  \node[lossblk, right=3.2cm of x] (kl) {\textbf{KL$[\pi_T \| \pi_S]$} \\ (at every step $t$)};
  \node[mrgblk, right=of kl] (out) {\textbf{Aligned} \\ (Policy $\pi_S^*$)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[datblk] (pairs) {\textbf{$(x, y)$ Pairs}};
  \node[frzblk, right=of pairs] (trm) {\textbf{Teacher RM $r_T$} \\ (frozen, large)};
  \node[grayblk, right=of trm] (scores) {\textbf{Teacher} \\ (Scores $r_T(x,y)$)};
  \node[trnblk, below=1.0cm of trm] (srm) {\textbf{Student RM $r_S$} \\ (small, trainable)};
  \node[lossblk, right=of srm] (loss) {\textbf{MSE +} \\ (Ranking Loss)};
  \node[mrgblk, right=of loss] (out) {\textbf{Compact} \\ (RM $r_S^*$)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (q) {\textbf{Query $q$}};
  \node[frzblk, above right=0.5cm and 1.2cm of q] (tret) {\textbf{Teacher} \\ \textbf{Retriever $q_T$} \\ (cross-encoder)};
  \node[trnblk, below right=0.5cm and 1.2cm of q] (sret) {\textbf{Student} \\ \textbf{Retriever $q_S$} \\ (bi-encoder)};
  \node[datblk, right=3.2cm of q] (corpus) {\textbf{Document} \\ (Corpus $\mathcal{C}$)};
  \node[lossblk, right=4.5cm of q] (kl) {\textbf{KL[$P_T \| P_S$]} \\ (over doc ranks)};
  \node[mrgblk, right=of kl] (out) {\textbf{Efficient} \\ (Retriever)};
...
\subsection{Mathematical Foundation}
QAT simulates low-bit precision during model training, allowing the optimizer to adjust parameters to compensate for quantization errors.
Let $w$ be the continuous weights. The quantized representation $\bar{w}$ is calculated as:
\begin{equation}
  \bar{w} = S \cdot \text{clip}\left( \text{round}\left( \frac{w}{S} \right) - Z, \, q_{\text{min}}, \, q_{\text{max}} \right)
\end{equation}
where $S$ is the quantization scale factor, $Z$ is the zero-point, and $[q_{\text{min}}, q_{\text{max}}]$ defines the integer boundary (e.g. $[-128, 127]$ for 8-bit quantization).
Because the rounding function has zero gradients almost everywhere ($\frac{\partial \text{round}(x)}{\partial x} = 0$), standard backpropagation fails. QAT resolves this by applying the Straight-Through Estimator (STE) during the backward pass, approximating the gradient of the rounding operator as identity:
\begin{equation}
  \frac{\partial \bar{w}}{\partial w} \approx \begin{cases} 1 & \text{if } w \in [w_{\text{min}}, w_{\text{max}}] \\ 0 & \text{otherwise} \end{cases}
\end{equation}
...
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (x) {\textbf{Input $x$}};
  \node[trnblk, right=of x] (fp32) {\textbf{FP32 Weights} \\ (master copy)};
  \node[grayblk, right=of fp32] (fakeq) {\textbf{Fake Quantize} \\ ($w \to \hat{w}$)};
  \node[frzblk, right=of fakeq] (fwd) {\textbf{Forward Pass} \\ (with $\hat{w}$)};
  \node[lossblk, right=of fwd] (loss) {\textbf{Task Loss} \\ ($\mathcal{L}$)};
  \node[trnblk, below=0.9cm of loss] (ste) {\textbf{STE Backward} \\ ($\partial\mathcal{L}/\partial w$)};
  \node[mrgblk, left=3.6cm of ste] (deploy) {\textbf{Deploy} \\ (INT$b$ Model)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[frzblk] (dense) {\textbf{Dense Model} \\ ($W$ (all weights))};
  \node[grayblk, right=of dense] (score) {\textbf{Importance} \\ (Scoring)};
  \node[orangeblk, right=of score] (mask) {\textbf{Binary} \\ (Mask $m$)};
  \node[trnblk, right=of mask] (sparse) {\textbf{Sparse Model} \\ ($W \odot m$)};
  \node[lossblk, below=0.9cm of sparse] (finetune) {\textbf{Fine-tune to} \\ (Recover Acc.)};
  \node[mrgblk, left=2.2cm of finetune] (out) {\textbf{Pruned} \\ (Model)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[frzblk] (fp32) {\textbf{FP32 Model} \\ (pretrained)};
  \node[datblk, right=of fp32] (cal) {\textbf{Calibration} \\ (Data $\mathcal{D}_{\text{cal}}$)};
  \node[grayblk, right=of cal] (hess) {\textbf{Hessian /} \\ (Scale Est.)};
  \node[orangeblk, right=of hess] (quant) {\textbf{Round Weights} \\ (to INT$b$)};
  \node[mrgblk, right=of quant] (int) {\textbf{INT$b$ Model} \\ (fast inference)};
...
\begin{tikzpicture}[node distance=0.5cm and 0.8cm]
  \node[datblk,minimum width=1.6cm] (q) {\textbf{Task $q$}};
  \node[trnblk,minimum width=1.6cm, right=0.7cm of q] (t1) {\textbf{Thought} \\ ($t_1$)};
  \node[orangeblk,minimum width=1.6cm, right=0.6cm of t1] (a1) {\textbf{Tool Call} \\ ($a_1$)};
  \node[cyanblk,minimum width=1.6cm, right=0.6cm of a1] (o1) {\textbf{Obs.} \\ ($o_1$)};
  \node[trnblk,minimum width=1.6cm, right=0.6cm of o1] (t2) {\textbf{Thought} \\ ($t_2$)};
  \node[orangeblk,minimum width=1.6cm, right=0.6cm of t2] (a2) {\textbf{Tool Call} \\ ($a_2$)};
  \node[mrgblk,minimum width=1.6cm, right=0.6cm of a2] (ans) {\textbf{Answer} \\ ($y$)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (x) {\textbf{User Request $x$}};
  \node[grayblk, above=0.7cm of x] (schema) {\textbf{Tool Schema $S$} \\ (JSON spec)};
  \node[frzblk, right=1.6cm of $(x)!0.5!(schema)$] (model) {\textbf{LLM} \\ ($P_\theta$)};
  \node[trnblk, right=of model] (json) {\textbf{JSON Call} \\ ($c=\{$name, args$\}$)};
  \node[lossblk, below=0.9cm of json] (val) {\textbf{Schema} \\ (Validation)};
  \node[mrgblk, left=1.4cm of val] (exec) {\textbf{Execute} \\ (Function)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.6cm and 1.0cm]
  \node[datblk] (task) {\textbf{Complex} \\ (Task $T$)};
  \node[frzblk,minimum height=1.4cm] (orch) at (3.2,0) {\textbf{Orchestrator} \\ ($\pi_0$)};
  \node[trnblk,minimum width=1.6cm] (s1) at (6.2,1.2) {\textbf{Specialist $\pi_1$} \\ (code)};
  \node[trnblk,minimum width=1.6cm] (s2) at (6.2,0.0) {\textbf{Specialist $\pi_2$} \\ (search)};
  \node[trnblk,minimum width=1.6cm] (s3) at (6.2,-1.2) {\textbf{Specialist $\pi_K$} \\ (math)};
  \node[mrgblk] (agg) at (9.2,0) {\textbf{Aggregate} \\ (Answer $y$)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (img) {\textbf{Image $I$}};
  \node[datblk, below=0.8cm of img] (txt) {\textbf{Text $x$}};
  \node[frzblk, right=of img] (venc) {\textbf{Visual} \\ \textbf{Encoder $f_v$} \\ (ViT/CLIP)};
  \node[grayblk, right=of venc] (proj) {\textbf{Projection} \\ (Layer $P$)};
  \node[trnblk, right=of txt] (emb) {\textbf{Text} \\ (Embeddings)};
  \node[frzblk, right=3.5cm of $(proj)!0.5!(emb)$] (llm) {\textbf{LLM} \\ ($f_l$)};
  \node[lossblk, right=of llm] (loss) {\textbf{LM Loss} \\ (on text)};
...
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (img) {\textbf{Image $I$}};
  \node[datblk, below=0.8cm of img] (expr) {\textbf{Referring} \\ (Expr.\ $e$)};
  \node[frzblk, right=1.6cm of $(img)!0.5!(expr)$] (vlm) {\textbf{VLM} \\ ($f_\theta$)};
  \node[trnblk, right=of vlm] (box) {\textbf{Predicted Box} \\ ($(x_1,y_1,x_2,y_2)$)};
  \node[lossblk, right=of box] (loss) {\textbf{L1 +} \\ (IoU Loss)};
  \node[datblk, above=0.7cm of loss] (gt) {\textbf{GT Box} \\ ($b^*$)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (q) {\textbf{Query $x$}};
  \node[frzblk, above right=0.5cm and 1.2cm of q] (ret) {\textbf{Retriever} \\ ($\mathcal{R}(x)$)};
  \node[datblk, right=1.2cm of q] (docs) {\textbf{Docs} \\ ($d_1..d_K$)};
  \node[trnblk, right=of docs] (llm) {\textbf{LLM} \\ ($P_\theta$)};
  \node[lossblk, right=of llm] (loss) {\textbf{Open-book} \\ (Loss)};
  \node[lossblk, below=0.8cm of llm] (cbloss) {\textbf{Closed-book} \\ (Loss)};
  \node[mrgblk, right=1.6cm of $(loss)!0.5!(cbloss)$] (out) {\textbf{RAG-tuned} \\ (Model)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (ep) {\textbf{Episode} \\ ($(x_t, y_t)$)};
  \node[trnblk, right=of ep] (write) {\textbf{Write Head} \\ ($w_\theta$)};
  \node[grayblk, right=of write, minimum height=2.0cm] (mem) {\textbf{Memory} \\ \textbf{Store} \\ ($\mathcal{M}$)};
  \node[datblk, below=1.2cm of ep] (query) {\textbf{Future} \\ (Query $x_{t'}$)};
  \node[trnblk, right=of query] (read) {\textbf{Read Head} \\ (dot-product)};
  \node[mrgblk, right=of read] (ctx) {\textbf{Retrieved} \\ (Context $v$)};
  \node[lossblk, right=of ctx] (loss) {\textbf{Contrastive} \\ (Memory Loss)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[datblk] (kg) {\textbf{Knowledge} \\ (Graph $\mathcal{G}$)};
  \node[grayblk, right=of kg] (emb) {\textbf{Entity/Relation} \\ (Embeddings)};
  \node[datblk, below=1.0cm of kg] (text) {\textbf{Text Tokens} \\ ($h_t$)};
  \node[trnblk, right=of emb] (cross) {\textbf{Cross-Attention} \\ (Injection)};
  \node[frzblk, right=of cross] (llm) {\textbf{LLM} \\ (Layers)};
  \node[lossblk, right=of llm] (loss) {\textbf{LM +} \\ (Fact-check Loss)};
  \node[mrgblk, below=0.9cm of loss] (out) {\textbf{Knowledge-} \\ (Grounded Model)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.1cm]
  \node[frzblk] (master) {\textbf{FP32 Master} \\ (Weights $W$)};
  \node[grayblk, right=of master] (cast) {\textbf{Cast to} \\ (FP16/BF16)};
  \node[trnblk, right=of cast] (fwd) {\textbf{Forward +} \\ (Backward (FP16))};
  \node[lossblk, right=of fwd] (scale) {\textbf{Loss Scale} \\ ($S \cdot \mathcal{L}$)};
  \node[trnblk, below=0.9cm of scale] (unscale) {\textbf{Unscale Grad} \\ ($g/S$)};
  \node[mrgblk, left=3.6cm of unscale] (upd) {\textbf{Adam Update} \\ (FP32 master)};
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.5cm and 0.8cm]
  \node[datblk,minimum width=1.4cm] (in) {\textbf{Input}};
  \node[trnblk,minimum width=1.4cm, right=0.6cm of in] (l1) {\textbf{Layer 1} \\ ({\tiny (stored)})};
  \node[grayblk,minimum width=1.4cm, right=0.5cm of l1] (l2) {\textbf{Layer 2} \\ ({\tiny (drop)})};
  \node[grayblk,minimum width=1.4cm, right=0.5cm of l2] (l3) {\textbf{Layer 3} \\ ({\tiny (drop)})};
  \node[trnblk,minimum width=1.4cm, right=0.5cm of l3] (l4) {\textbf{Layer 4} \\ ({\tiny (stored)})};
  \node[grayblk,minimum width=1.4cm, right=0.5cm of l4] (l5) {\textbf{Layer 5} \\ ({\tiny (drop)})};
  \node[mrgblk,minimum width=1.4cm, right=0.5cm of l5] (out) {\textbf{Output}};
...
\subsection{Mathematical Foundation}
FlashAttention accelerates attention computation by tiling input blocks in SRAM and utilizing online softmax reduction to avoid materializing the massive $N \times N$ attention matrix in HBM.
Let $Q, K, V$ be split into blocks $Q_i, K_j, V_j$ of sizes $B_c, B_r$. The online softmax computes running scaling statistics. For a query block $Q_i$:
At step $j$ (processing key block $K_j$), we compute raw attention scores:
\begin{equation}
  S^{(j)} = \frac{Q_i K_j^\top}{\sqrt{d}}
\end{equation}
We update running row-wise maximums $m^{(j)}$ and denominators $d^{(j)}$:
\begin{align}
  \tilde{m}^{(j)} &= \max(S^{(j)}) \\
  m^{(j)} &= \max(m^{(j-1)}, \, \tilde{m}^{(j)}) \\
  \tilde{d}^{(j)} &= \sum_k \exp(S^{(j)}_k - m^{(j)}) \\
  d^{(j)} &= d^{(j-1)} \exp(m^{(j-1)} - m^{(j)}) + \tilde{d}^{(j)}
\end{align}
The running attention output block $O_i$ is updated dynamically using:
\begin{equation}
  O_i^{(j)} = \frac{d^{(j-1)} \exp(m^{(j-1)} - m^{(j)}) \cdot O_i^{(j-1)} + \exp(\tilde{m}^{(j)} - m^{(j)}) \cdot \tilde{P}^{(j)} V_j}{d^{(j)}}
\end{equation}
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.7cm and 1.0cm]
  \node[datblk] (qkv) {\textbf{$Q, K, V$} \\ (HBM)};
  \node[grayblk, right=of qkv] (tile) {\textbf{Load Tile} \\ \textbf{$B_r \times B_c$} \\ (to SRAM)};
  \node[trnblk, right=of tile] (local) {\textbf{Local Softmax} \\ (+ $V$ accumulate)};
  \node[grayblk, right=of local] (write) {\textbf{Write partial} \\ (output to HBM)};
  \node[mrgblk, below=0.9cm of $(tile)!0.5!(write)$] (final) {\textbf{Final $O$} \\ (no full $N^2$ stored)};
...
\subsection{Mathematical Foundation}
FSDP shards all three components of the model state (Parameters $\Psi$, Gradients $G$, and Optimizer States $O$) across all $N$ available GPUs.
Let the model parameters occupy $M_{\text{param}} = 2\Psi$ bytes in 16-bit precision, gradients occupy $M_{\text{grad}} = 2\Psi$ bytes, and Adam optimizer states occupy $M_{\text{opt}} = 12\Psi$ bytes (storing FP32 copies of weights, first moments, and second moments).
Under standard Distributed Data Parallel (DDP), the parameters, gradients, and optimizer states are fully duplicated on each GPU:
\begin{equation}
  M_{\text{baseline}} = 2\Psi + 2\Psi + 12\Psi = 16\Psi \text{ bytes}
\end{equation}
FSDP shards these states across $N$ GPUs. The peak memory occupancy per GPU is:
\begin{equation}
  M_{\text{FSDP}} = \underbrace{\frac{2\Psi}{N}}_{\text{sharded params}} + \underbrace{\frac{2\Psi}{N}}_{\text{sharded grads}} + \underbrace{\frac{12\Psi}{N}}_{\text{sharded optimizer}} + \underbrace{2\Psi_{\text{block}}}_{\text{reconstructed block}}
\end{equation}
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.6cm and 1.0cm]
  \node[grayblk,minimum width=1.4cm] (d1) {\textbf{Device 1} \\ ($\theta^{(1)}$)};
  \node[grayblk,minimum width=1.4cm, right=0.4cm of d1] (d2) {\textbf{Device 2} \\ ($\theta^{(2)}$)};
  \node[grayblk,minimum width=0.8cm, right=0.4cm of d2] (dots) {\textbf{$\cdots$}};
  \node[grayblk,minimum width=1.4cm, right=0.4cm of dots] (dD) {\textbf{Device $D$} \\ ($\theta^{(D)}$)};

  \node[trnblk, below=0.9cm of d2] (ag) {\textbf{All-Gather} \\ ($\theta_{\text{full}}$)};
  \node[frzblk, right=1.6cm of ag] (fwd) {\textbf{Layer} \\ (Forward)};
  \node[lossblk, right=of fwd] (bwd) {\textbf{Backward} \\ ($g_{\text{full}}$)};
  \node[orangeblk, below=0.9cm of bwd] (rs) {\textbf{Reduce-Scatter} \\ ($g^{(d)}$)};
  \node[mrgblk, left=2.2cm of rs] (upd) {\textbf{Local} \\ (Adam Update)};
...
1. **ZeRO-1 (Optimizer State Partitioning)**: The optimizer states (e.g. Adam states) are sharded across $N_{\text{dp}}$ data-parallel ranks. Parameters and gradients remain fully replicated. Peak memory per GPU is:
\begin{equation}
  M_{\text{ZeRO-1}} = 2\Psi + 2\Psi + \frac{12\Psi}{N_{\text{dp}}}
\end{equation}
...
\begin{equation}
  M_{\text{ZeRO-2}} = 2\Psi + \frac{2\Psi}{N_{\text{dp}}} + \frac{12\Psi}{N_{\text{dp}}}
\end{equation}
...
\begin{equation}
  M_{\text{ZeRO-3}} = \frac{2\Psi}{N_{\text{dp}}} + \frac{2\Psi}{N_{\text{dp}}} + \frac{12\Psi}{N_{\text{dp}}}
\end{equation}
...
\begin{adjustbox}{max width=\linewidth}
\begin{tikzpicture}[node distance=0.5cm and 0.9cm]
  \node[datblk,minimum width=2.2cm] (full) {\textbf{Standard} \\ \textbf{DDP:} \\ ($16|\theta|$ per GPU)};
  \node[grayblk,minimum width=2.2cm, right=0.7cm of full] (z1) {\textbf{ZeRO-1:} \\ \textbf{Shard Opt States} \\ ($12|\theta|+ 4|\theta|/D$)};
  \node[trnblk,minimum width=2.2cm, right=0.7cm of z1] (z2) {\textbf{ZeRO-2:} \\ \textbf{+ Shard Gradients} \\ ($4|\theta|+8|\theta|/D$)};
  \node[mrgblk,minimum width=2.2cm, right=0.7cm of z2] (z3) {\textbf{ZeRO-3:} \\ \textbf{+ Shard Params} \\ ($12|\theta|/D$)};
