...
└── evaluator.py: run_hybrid_reconciliation(table_name, mode="TIER1_SAMPLE")
    ├── sampler.py: stream_sample_checksums(table_name, sample_rate)
    │   └── models.py: BlockChecksumResult(pk_range, source_hash, target_hash)
    │
    ├── [Tier 2 Scheduled] sweeper.py: run_periodic_full_sweep(table_name, iops_cap)
    │   └── models.py: FullSweepChunkResult(scanned_rows, mismatch_rows)
    │
    ├── evaluator.py: eval_reconciliation_parity(checksum_result, sweep_result)
    │   └── models.py: ReconciliationSweepResult(is_matched, total_mismatches)
    │
    └── observability/sweep_metrics.py: record_sweep_telemetry(sweep_result)
```
