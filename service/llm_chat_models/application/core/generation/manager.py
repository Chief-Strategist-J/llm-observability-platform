...
class ModelManager:
    def __init__(self):
        # Currently defaults to Ollama, can be made configurable
    ...
    async def download_model(self, model_name: str) -> bool:
        info = get_model_info(model_name)
        if not info:
            log_event("download_model_failed", reason="unknown_model", model=model_name)
    ...
    async def start_model(self, model_name: str) -> bool:
        ...
        if status.get("status") != "available":
            log_event("start_model_failed", reason="not_downloaded", model=model_name)
            return False
    ...
    async def generate_response(self, model_name: str, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        return await self.adapter.generate_response(info["name"], prompt, **kwargs)
