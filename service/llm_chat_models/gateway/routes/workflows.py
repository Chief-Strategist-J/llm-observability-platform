...
from gateway.schemas.workflows import WorkflowRunRequest, WorkflowActionRequest, WorkflowEventRequest, WorkflowResponse, WorkflowListResponse
...
async def list_workflows(status: Optional[str]=None):
    ...
    if status:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid status: '{status}'. Valid: {[s.value for s in WorkflowStatus]}")
    results = engine.list_workflows(status=status_filter)
    return WorkflowListResponse(workflows=[WorkflowResponse(**r.to_dict()) for r in results], total=len(results))
...
@trace_with_details(get_tracer())
async def approve_workflow(workflow_id: str, request: WorkflowActionRequest):
...
@trace_with_details(get_tracer())
async def reject_workflow(workflow_id: str, request: WorkflowActionRequest):
...
async def cancel_workflow(workflow_id: str):
    ...
    if not cancelled:
        raise HTTPException(status_code=400, detail=f'Workflow {workflow_id} cannot be cancelled (not running or not found)')
    return {'success': True, 'message': f'Workflow {workflow_id} cancelled'}

@router.post('/{workflow_id}/events')
@trace_with_details(get_tracer())
async def send_workflow_event(workflow_id: str, request: WorkflowEventRequest):
    log_event('api_send_workflow_event', workflow_id=workflow_id, event=request.event_name)
    engine = get_engine()
    await engine.send_event(workflow_id, request.event_name, request.payload)
    return {'success': True, 'message': f"Event '{request.event_name}' sent to {workflow_id}"}